...
exports.addSubcategory = async (req, resp) => {
    ...
    if (!!validation)
        ...
        let prevSubcategory = await Subcategory.findOne({ $and: [{ name: formData.name }, { categoryId: formData.categoryId }] })
        if (prevSubcategory)
    ...
    if (!!validation)
        ...
        await Subcategory.findOne(query).then(async res => {
            if (!!res) {
                ...
                let id = res._id
                let prevSubcategory = await Subcategory.findOne({ $and: [{ name: res.name }, { categoryId: res.categoryId }, { _id: { $ne: id } }] })
                if (prevSubcategory)
                    resp.send({ success: false, status: 409, message: "Subcategory already exists with same name" })
                else
                    res.save().then(res => {
